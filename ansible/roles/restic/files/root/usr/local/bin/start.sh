...
if [ -n "$RCLONE_REMOTE_NAME" ]; then
  if [ -n "$RCLONE_REMOTE_LOCATION" ]; then
    ...
    # ${RCLONE_SERVE_ARGS:-}
    rclone serve restic ${RCLONE_REMOTE_NAME}:${RCLONE_REMOTE_LOCATION} --addr 0.0.0.0:8080 --log-file /config/rclone/log/restic-rclone.log --config=${RCLONE_CONFIG_LOCATION:-/config/rclone/rclone.conf} \
    ${RCLONE_SERVE_ARGS:-}
  else
